...
class PurchaseCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_form(self, form_class=None):
        ...
        try:
            if self.request.user.profile.default_store:
                form.fields['store'].initial = self.request.user.profile.default_store
        except Exception:
            pass
        return form    
...
class PurchaseUpdateView(LoginRequiredMixin, UpdateView):
        ...
        ctx["existing_lines"] = [
            {
                ...
                "category_name": d.item.category.name if d.item.category else "",
                "quantity": d.quantity,
                "price": float(d.price),
                "stock": int(d.item.stock or 0),
            }
            for d in self.object.purchase_details.select_related("item", "item__category")
        ]
    ...
    def form_valid(self, form):
        """
        Delta-based update:
        - Build new_details_map from incoming payload
        - Lock and fetch existing details via select_for_update()
        - Delete removed items (call delete() -> model will rollback stock)
        - For existing items: compute delta, adjust Item.stock manually under select_for_update,
          then update the PurchaseDetail row via QuerySet.update(...) to bypass model.save() stock logic
        - For new items: create normally (model save will add stock)
        """
        purchase = form.save(commit=False)
        details = json.loads(self.request.POST.get("details_json", "[]"))
        ...
        try:
            with db_transaction.atomic():
                # Save header (if any header fields were changed)
                purchase.save()

                # Build map of incoming details keyed by item_id
                new_details_map = {}
                for ln in details:
                    # ensure item_id is int for consistent mapping
                    new_details_map[int(ln["item_id"])] = ln

                # Lock and fetch existing lines
                existing_qs = purchase.purchase_details.select_for_update().select_related("item")
                existing_details = {int(d.item_id): d for d in existing_qs if d.item_id is not None}

                # 1) Delete removed items (those in existing but not in incoming)
                for item_id, old_detail in list(existing_details.items()):
                    if item_id not in new_details_map:
                        # delete will rollback stock via model.delete()
                        old_detail.delete()
                        existing_details.pop(item_id, None)
                ...
                for ln in details:
                    ...
                    if item_id in existing_details:
                        # Update existing - calculate delta
                        detail = existing_details[item_id]
                        old_qty = int(detail.quantity or 0)
                        delta = qty - old_qty

                        if delta != 0:
                            # Adjust Item stock manually under lock
                            it = Item.objects.select_for_update().get(pk=item.id)
                            it.stock = int(it.stock or 0) + int(delta)
                            it.save(update_fields=["stock"])

                        # Update DB row directly to bypass PurchaseDetail.save() stock logic
                        PurchaseDetail.objects.filter(pk=detail.pk).update(
                            quantity=qty,
                            price=price,
                            total_detail=float(line_total)
                        )
                    else:
                        # New detail - create normally (model's save will apply stock for create)
                        PurchaseDetail.objects.create(
                            purchase=purchase,
                            item=item,
                            quantity=qty,
