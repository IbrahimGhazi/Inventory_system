...
# ---------------------------------------------------------------------
# Helper functions used by the rest of your module (kept from original)
# ---------------------------------------------------------------------
...
# --- per-model sync helpers (kept as-is) --------------------------------
# ... (your sync_user/sync_profile/sync_customer/... functions remain unchanged)
# For brevity I'm not duplicating them here; keep your original functions below this line
# (You should copy all your original per-model sync_* and delete_* functions here.)
# -------------------------------------------------------------------------

# ===============================================================================
# FULL SYNC (push) - keep your original implementation (unchanged)
# ===============================================================================
def sync_all_data():
    # (keep your original sync_all_data function exactly as you had it)
    if not _is_sync_enabled():
            ...
            for row in rows:
                try:
                    ...
                    if pk is not None:
                        ...
                        obj, created = model.objects.update_or_create(id=pk, defaults=row)
                        restored += 1
                    else:
                        # no id -> try to create
